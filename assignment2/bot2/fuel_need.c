...
int fuel_need(struct bot *b ,int distance){
    ...
    while (distance2 == 0 || forwards != start) {
        if (forwards->type == LOCATION_PETROL_STATION && forwards->quantity >= b->fuel_tank_capacity/2) {
            distance_final = distance2;
            break;
        }
        if (backwards->type == LOCATION_PETROL_STATION && forwards->quantity >= b->fuel_tank_capacity/2) {
            distance_final = -distance2;
            break;
        }
