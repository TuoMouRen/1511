...
// if in a fuel station and fuel is no full 
    if (now->type == LOCATION_PETROL_STATION && b->fuel != b->fuel_tank_capacity) {
        *action = ACTION_BUY;
...
// cargo has commodity:
        ...
        int distance2/*to nearst buyer*/ =nearest_buyer(b);
        if (now->type == LOCATION_DUMP && distance2 == NO_BUYER) {  //"NO_BUYER" means if no buyer meet the conditions(in nearest_buyer function)
            *action = ACTION_DUMP;      
