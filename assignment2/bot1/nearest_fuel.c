...
int nearest_fuel(struct bot *b) {
    ...
    while (distance == 0 || forwards != start) {
        if (forwards->type == LOCATION_PETROL_STATION && forwards->quantity >= b->fuel_tank_capacity ) {
            n = distance;
            break;
        }
        if (backwards->type == LOCATION_PETROL_STATION && backwards->quantity >= b->fuel_tank_capacity) {
            n = -distance;
            break;
        }
